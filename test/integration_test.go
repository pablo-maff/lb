...
func TestMain(m *testing.M) {
	...
	out, err := exec.Command("go", "build", "-o", bin, "lb/cmd/lb").CombinedOutput()
	if err != nil {
...
func startLB(t *testing.T, backends []string, port int) {
	t.Helper()
...
func TestIntegration_RoundRobin(t *testing.T) {
	...
	if hits[0].Load() == 0 || hits[1].Load() == 0 {
		t.Fatalf("expected both backends hit, got %d and %d", hits[0].Load(), hits[1].Load())
	}
}
